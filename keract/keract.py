...
def get_gradients_of_activations(model, x, y, layer_name=None, output_format='simple', nested=False, sep='__'):
    """
    ...
    - 'nested': If specified, will move recursively through the model definition to retrieve nested layers.
                Recursion ends at leaf layers of the model tree or at layers with their name specified in layer_name.
                ...
                ... yields a dictionary with keys 'layer1__conv1', ..., 'layer1__fc1', 'layer2__fc2'.
                If layer_name = ['layer2__fc2'] is specified, the dictionary will only hold one key 'layer2__fc2'.

                The layer names are generated by joining all layers from top level to leaf level with a separator.
                The separator is '__' by default.
    - 'sep': The separator to use for nested layer names if 'nested' is set to True.
    :return: Dict {layer_name (specified by output_format) -> activation of the layer output/node (Numpy array)}.
    """
    nodes = _get_nodes(model, output_format, nested=nested, layer_names=layer_name, sep=sep)
    return _get_gradients(model, x, y, nodes)
...
def _get_nodes(module, output_format, nested=True, layer_names=[], prefix='', sep='__'):
    ...
    if bool(layer_names) and prefix in layer_names:
        node_dict.update({prefix: module})
    elif has_layers:
        for m in module._layers:
            key = n_(m, output_format_=output_format)
            module_name = key if prefix == '' else '{}{}{}'.format(prefix, sep, key)
            if nested:
                ...
                nodes = OrderedDict([(module_name, m)])
            if nodes:
...
def _is_input_layer(layer, key):
    return 'input_' in key or isinstance(layer, InputLayer)


def get_activations(model, x, layer_name=None, nodes_to_evaluate=None,
                    output_format='simple', nested=False, sep='__', auto_compile=True):
    """
    ...
    if nodes_to_evaluate is None:
        if nested:
            nodes = _get_nodes(model, output_format, layer_names=layer_name, sep=sep)
        else:
            nodes = [layer for layer in model.layers if layer.name == layer_name or layer_name is None]
    else:
    ...
    if nested:
        input_layer_outputs_d, layer_outputs_d = OrderedDict(), OrderedDict()
        [input_layer_outputs_d.update({key: node.output}) if _is_input_layer(node, key) else layer_outputs_d.update({key: node.output}) for key, node in nodes.items()]
        input_layer_outputs = input_layer_outputs_d.values()
        layer_outputs = layer_outputs_d.values()
    else:
        [input_layer_outputs.append(node.output) if _is_input_layer(node, node.name) else layer_outputs.append(node.output) for node in nodes]
    activations = _evaluate(model, layer_outputs, x, y=None, auto_compile=auto_compile)

    def craft_output(output_format_):
        if nested:
            activations_dict = OrderedDict(zip(layer_outputs_d.keys(), activations))
            activations_inputs_dict = OrderedDict(zip(input_layer_outputs_d.keys(), x))
        else:
